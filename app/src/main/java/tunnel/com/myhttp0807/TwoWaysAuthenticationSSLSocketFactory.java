...
public class TwoWaysAuthenticationSSLSocketFactory {
	...
	public static SSLSocketFactory getSSLSocketFactory (Context ctx)
		...
		SSLContext context = SSLContext.getInstance("TLS");//TLSv1
		...
		try{
	        ...
	        TrustManagerFactory trustManager = TrustManagerFactory.getInstance(TrustManagerFactory.getDefaultAlgorithm()); 
	           ...
	           //取得BKS密库实例  
	            ...
	            KeyStore kks= KeyStore.getInstance(KEY_STORE_TYPE_P12);  

	           KeyStore tks = KeyStore.getInstance(KEY_STORE_TYPE_BKS);  

	        //加客户端载证书和私钥,通过读取资源文件的方式读取密钥和信任证书 

	           kks.load(pContext.getResources().getAssets().open(keyStoreFileName),keyStorePassword.toCharArray()); 

	          tks.load(pContext.getResources().getAssets().open(trustStoreFileName),trustStorePassword.toCharArray());  

	       //初始化密钥管理器  

	         keyManager.init(kks,keyStorePassword.toCharArray());
		...
		SSLSocketFactory socketFactory =context.getSocketFactory();//new NoSSLv3SocketFactory(context.getSocketFactory());
